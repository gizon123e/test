...
module.exports = {
    getDataAlamatDistributtor: async (req, res, next) => {
        try {
            const data = await AlamatPenanggungJawab.find()
    ...
    getDataAlamatDistributtorById: async (req, res, next) => {
        try {
            const dataByID = await AlamatPenanggungJawab.findOne({ _id: req.params.id })
    ...
    createDataAlamatDistributtor: async (req, res, next) => {
        try {
            const { id_penanggung_jawab_distributor, alamat_lengkap, latitude, longitude } = req.body

            const data = await AlamatPenanggungJawab.create({ id_penanggung_jawab_distributor, alamat_lengkap, latitude, longitude })
    ...
    updateDataAlamatDistributtor: async (req, res, next) => {
        try {
            ...
            const data = await AlamatPenanggungJawab.findByIdAndUpdate({ _id: req.params.id }, { id_penanggung_jawab_distributor, alamat_lengkap, latitude, longitude }, { new: true })
    ...
    deleteDataAlamataDistributtor: async () => {
        try {
            const data = await AlamatPenanggungJawab.findOne({ _id: req.params.id })
            if (!data) return res.status(404).json({ message: "data Not Found" })

            await AlamatPenanggungJawab.deleteOne({ _id: req.params.id })
