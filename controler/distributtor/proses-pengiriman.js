...
module.exports = {
  ...
  mulaiPenjemputanPerusahaan: async (req, res, next) => {
    try {
      const { id_address, latitude, longitude, id_konsumen, id_pengemudi, id_kendaraan, id_user_distributor } = req.body;
  ...
  sudahDiJemputPerusahaan: async (req, res, next) => {
    try {
      const { id_address, latitude, longitude, id_konsumen, total_qty, id_user_distributor } = req.body;
      if (!total_qty || !id_address || !latitude || !longitude || !id_konsumen) return res.status(400).json({ message: "data total_qty, latitude, longitude, id_konsumen dan id_address harus di isi" });
  ...
  mulaiPengirimanPerusahaan: async (req, res, next) => {
    try {
      const distri = await Distributtor.exists({ userId: req.body.id_user_distributor });
  ...
  pesasanSelesaiPerusahaan: async (req, res, next) => {
    try {
      const { id_address, latitude, longitude, id_konsumen, id_user_distributor } = req.body;
      const files = req.files;
      const images = files ? files.images : null;
