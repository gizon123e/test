...
module.exports = {
    getDetailTransaksi: async(req, res, next) => {
        try {
            ...
            // const finalData = []
            // pengiriman.productToDelivers.map(prod => fixedDataProduct.dataProduct.filter)
            const productIds = pengiriman.productToDelivers.map(prd => prd.productId)
            const filteredProd = fixedDataProduct.dataProduct.filter(prd => productIds.includes(prd._id) ).map()
