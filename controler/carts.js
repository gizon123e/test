            ...
            for (let key of keys){
                ...
                switch(storeMap[key].role){
                    ...
                    case "supplier":
                        ...
                        break;
                    case "produsen":
                        detailToko = await TokoProdusen.findOne({userId: storeMap[key].id}).select('namaToko userId');
                        break;
                }
                finalData.push({
            ...
            for(const key of keys){
                switch(store[key].role){
                    case "vendor":
                        ...
                        break;
                    case "supplier":
                        detailToko = await TokoSupplier.findOne({userId: store[key].id});
                        break;
                    case "produsen":
                        detailToko = await TokoProdusen.findOne({userId: store[key].id});
                        break;
                }
                finalData.push({
                    nama_toko: detailToko.namaToko,
