...
module.exports = {
    ...
    getDetailToko: async(req, res, next) => {
        try {
            ...
            if(req.user.role !== "vendor") return res.status(403).json({message: "Invalid Request"})
    ...
    getAllProsesPengiriman: async (req, res, next) => {
        try {   
            ...
            const toko = await Toko.findOne({userId: req.user.id})
    ...
    getRingkasan: async(req, res, next) => {    
        try {
            const toko = await TokoSupplier.exists({userId: req.user.id});
